...
export default function Index({ auth, inviteStatus, page }) {
    ...
    const { data, last_page, total } = inviteStatus;
    ...
    const handleStore = () => {
        router.post("/invite-status", formData, {
            preserveScroll: true,
            onError: (errors) => {
                if (errors.name) {
    ...
    const handleUpdate = (id) => {
        router.put(`/invite-status/${id}/update`, formData, {
            preserveScroll: true,
            onSuccess: () => {
                setToastData({
    ...
    const deleteStatus = () => {
        router.delete(`/invite-status/${dataStatus.id}/delete`, {
            preserveScroll: true,
            onSuccess: () => {
                setToastData({
        ...
        if (total > 0) {
            return (
                <div className="w-full ms-2 me-2 mt-4">
                    <LinkNumber
                        href={route("invite-status.index", { page: 1 })}
                        active={route().current("invite-status.index", {
                            page: 1,
                        })}
                    ...
                    {rows.map((value) => {
                        return (
                            <LinkNumber
                                key={value}
                                href={route("invite-status.index", {
                                    search: search,
                                    page: value,
                                })}
                                active={route().current("invite-status.index", {
                    ...
                    <LinkNumber
                        href={route("invite-status.index", { page: last_page })}
                        active={route().current("invite-status.index", {
                            page: last_page,
                        })}
                        className="bg-sky-500 pt-2 pb-1 px-3 mx-1 text-white rounded-md"
                                                    ...
                                                    <td>
                                                        <div className="inline-flex">
                                                            <svg
                                                                ...
                                                                stroke="currentColor"
                                                                className="w-6 h-6 bg-sky-700 p-1 m-1 text-white rounded-sm cursor-pointer"
                                                                onClick={() =>
                                                                    handleEdit(
                                                                        stage.id
                                                                    )
